...
class Petri_build:
    ...
    def __init__(self, instance_id,
        ...
        self.n_jobs, self.n_machines, self.n_tools, self.max_bound,self.max_ops = self.instance.specs
    ...
    def filter_nodes(self, node_role , node_type=None):
        """
        ...
        if node_type =="p":
            return ([place.uid for place in self.places.values() if place.role == node_role])
        elif node_type =="t":
            return [ transition.uid for transition in self.transitions.values() if transition.role == node_role]
        
        else :
           return ( [
               place.uid for place in self.places.values() if place.role == node_role
               ]+[ transition.uid for transition in self.transitions.values() if transition.role == node_role])
    ...
    def add_tokens(self):
        ...
        for job, uid in enumerate(self.filter_nodes(jobs_source), start=1): 
            previous_machine = 0
            last_op=False
            ...
            try : # only add token to the operation in the instance  (for dynamic variant ) 
                ...
                for i, (machine, tool, process_time) in enumerate(job_sequence, start=1):
                    ...
                    agv_loaded =self.instance.get_time(previous_machine, machine, time_type=1) if self.n_agv else 0
                    tt_loaded =self.instance.get_time(previous_machine, machine, time_type=2) if self.n_tt else 0
    ...
    def cache_nodes(self):
        """Cache places and transitions in the simulation."""
        ...
        cached_nodes = {
            ...
            "buffer_places": [p for p in places if p.type == "b"],
            "process_places": [p for p in places if p.type == "p"],
            "sorting_places": [p for p in places if p.type == "s"],
            "flow_places" : [p for p in places if p.type not in  ["d","f"]], # not  delivery and not a flag 
            "delivery_places" : [p for p in places if p.type  =="d"],  #delivery 
            
            
            "job_stock" : [p for p in places if p.role == "job_stock"],
            "job_idle": [p for p in places if p.role == "job_idle"],
            "machine_processing": [p for p in places if p.role == "machine_processing"],
            

           
            "job_feed": [t for t in transitions if t.uid in self.filter_nodes("job_feed", "t")],
            "machine_start": [t for t in transitions if t.uid in self.filter_nodes("machine_start", "t")],
            "auto_transitions": [t for t in transitions if t.type == "a"],
            "controllable_transitons": [t for t in transitions if t.type == "c"],
        }
        return  cached_nodes
    
    
    def clear_petri(self):
        for place in self.places.values():
            place.token_container = []
    
    def save_token_state(self):
        token_state = {}
        for place_uid, place in self.places.items():
            token_state[place_uid] = []
            for token in place.token_container:
                token_state[place_uid].append({
                    "uid": token.uid,
                    "rank": token.rank,
                    "type_": token.type_,
                    "role": token.role,
                    "color": list(token.color),
                    "time_features": token.time_features,
                    "machine_sequence": list(token.machine_sequence),
                    "last_op": token.last_op,
                    "logging": copy.deepcopy(token.logging),  #  DEEP COPY
                })
        return token_state
            


    def load_token_state(self, token_state):
        """Load token states into the Petri net."""
        for place in self.places.values():
            place.token_container.clear()
    
        for place_uid, tokens_data in token_state.items():
            if place_uid not in self.places:
                continue
            for token_data in tokens_data:
                token = Token(
                    uid=token_data.get("uid"),
                    rank=token_data.get("rank"),
                    type_=token_data.get("type_"),
                    role=token_data.get("role"),
                    color=tuple(token_data.get("color")),
                    time_features=token_data.get("time_features"),
                    machine_sequence=tuple(token_data.get("machine_sequence")),
                    last_op=token_data.get("last_op")
                )
                #  DEEP COPY logging to avoid shared references
                token.logging = copy.deepcopy(token_data.get("logging", {}))
                self.places[place_uid].token_container.append(token)
    ...
    def create_petri(self,show_flags=False,show_develop=False):
        ...
        nodes_layers = [
            ...
            ("trans", "a", "machine_start",  False, False, True, self.n_machines),  # random control
        ...
        if self.standby:
            nodes_layers += [
               ...
               ("trans", "c", "standby",  False, False, True, 1),  
            ]
        ...
        if self.random_arrival:
             ...
             nodes_layers += [
                ("place", "b", "job_stock",  True, False, True, self.n_jobs), 
                ("trans", "r", "job_feed",  True, False, True, self.n_jobs),  # random control 
             ]

             layers_to_connect += [
                 
                 ("job_stock", "job_feed", "p2t", False),
                 ("job_feed", "job", "t2p", False),
             ]
        ...
        if self.n_agv > 0 :
            ...
            nodes_layers += [
                ...
                ("trans", "a", "agv_select",  True, False, True,self.n_agv),
            ...
            layers_to_connect += [
                ...
                ("agv_finish","agv_ready", "t2p", False),
...
if __name__ == "__main__":
    ...
    token_state=petri.save_token_state()

    petri.clear_petri()
    
    
    petri.load_token_state(token_state)
